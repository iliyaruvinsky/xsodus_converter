...
echo [1/13] Killing Python processes by name...
taskkill /F /IM python.exe >nul 2>&1
taskkill /F /IM pythonw.exe >nul 2>&1
echo    ✓ Killed by process name

echo.
echo [2/13] Finding processes using port 8000...
...
echo.
echo [3/13] Waiting for connections to clear (attempt 1)...
...
echo.
echo [4/13] Additional cleanup - killing uvicorn processes...
...
echo.
echo [5/13] Waiting for connections to clear (attempt 2)...
...
echo.
echo [6/13] Final port check and PID cleanup...
for /f "tokens=5" %%a in ('netstat -ano ^| findstr :8000') do (
...
:port_free
echo.
echo [7/13] Verifying port 8000 is free...
netstat -ano | findstr :8000 >nul
if %ERRORLEVEL% EQU 0 (
...
echo.
echo [10/13] Reinstalling core package...
cd core
pip install -e . --quiet
if %ERRORLEVEL% EQU 0 (
...
echo.
echo [11/13] Reinstalling xml-to-sql pipeline...
...
echo.
echo [12/13] Verifying package installation...
python -c "from x2s_core.parser import scenario_parser; print('    ✓ Core loaded')" 2>&1
python -c "from src.renderer import renderer; print('    ✓ xml-to-sql loaded')" 2>&1

echo.
echo [13/13] Starting web server...
