...
def _parse_mappings(node_el: etree._Element) -> Tuple[List[AttributeMapping], List[Tuple[str, Dict[str, AttributeMapping]]]]:
    ...
    for input_el in _find_children(node_el, "input"):
        ...
        for mapping_el in _find_children(input_el, "mapping"):
            ...
            # BUG-036: Handle ConstantAttributeMapping for UNION nodes
            # These have target and value attributes, but no source attribute
            # Example: <mapping xsi:type="Calculation:ConstantAttributeMapping" target="CODE_TYPE" value="START_ROUTINE"/>
