...
def _render_aggregation(ctx: RenderContext, node: AggregationNode) -> str:
    ...
    for agg_spec in node.aggregations:
        ...
        # SUM and AVG on non-numeric columns need TO_INTEGER cast in HANA
        if agg_func in ('SUM', 'AVG', 'STDDEV', 'VAR') and ctx.database_mode == DatabaseMode.HANA:
            # Check data type from agg_spec or expression
            data_type = agg_spec.data_type or agg_spec.expression.data_type
            if data_type:
                # Handle string data types (can be str or DataTypeSpec)
    ...
    if input_id in ctx.scenario.data_sources:
        ...
        if ctx.database_mode == DatabaseMode.HANA and ds.source_type == DataSourceType.CALCULATION_VIEW:
            ...
            if package:
                ...
                # BUG-037: cv_name may contain path prefix from resourceUri (e.g., "Assesment/ASSESSMENT_REPORT")
                # Extract just the view name (after last "/") when no package mapping exists
                ...
                return f'"_SYS_BIC".{_quote_identifier(view_name_only)}'
...
def _generate_view_statement(view_name: str, mode: DatabaseMode, scenario: Optional[Scenario] = None) -> str:
    ...
    if mode == DatabaseMode.SNOWFLAKE:
        ...
        # BUG-041 REVERTED: IF EXISTS not supported in user's HANA version (pre-SPS03)
        # - If view doesn't exist: User gets [321] error on DROP - skip it and run CREATE only
        # - If view exists: DROP succeeds, then CREATE works
        # CASCADE drops dependent objects (required for views with dependencies)
