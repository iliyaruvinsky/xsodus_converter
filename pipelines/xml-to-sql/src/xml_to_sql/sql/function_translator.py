...
def _substitute_placeholders(text: str, ctx) -> str:
    """Replace $$client$$, $$language$$, and input parameter placeholders.
    ...
    Input parameters ($$paramName$$) are replaced with their default values
    from the XML localVariables section, since standard SQL VIEWs don't support
    the $$param$$ placeholder syntax used in Calculation Views.
    """
    mode = getattr(ctx, "database_mode", DatabaseMode.SNOWFLAKE)
    ...
    # Replace input parameters ($$paramName$$) with their default values
    # These come from <localVariables><variable> elements in the XML
    scenario = getattr(ctx, "scenario", None)
    if scenario and hasattr(scenario, "variables"):
        for var in scenario.variables:
            if var.variable_id and var.default_value is not None:
                placeholder = f"$${var.variable_id}$$"
                result = result.replace(placeholder, str(var.default_value))
...
def _build_replacement(rule: FunctionRule, args: Sequence[str], ctx) -> Optional[str]:
    ...
    Recursively applies catalog rewrites to each argument to handle nested calls.
    E.g., format(format(x)) -> TO_VARCHAR(TO_VARCHAR(x))
    """

    handler = rule.handler.lower()

    # Recursively process each argument to handle nested function calls
    processed_args = [_apply_catalog_rewrites(arg, ctx) for arg in args]

    if handler == "template" and rule.template:
        try:
            return rule.template.format(*processed_args)
        except IndexError:
            return None

    if handler == "rename" and rule.target:
        return f"{rule.target}({', '.join(processed_args)})"

    if handler == "regexp_like":
        if not processed_args:
            return None
        target = processed_args[0]
        pattern = processed_args[1] if len(processed_args) > 1 else "'*'"
        translated_pattern = (
