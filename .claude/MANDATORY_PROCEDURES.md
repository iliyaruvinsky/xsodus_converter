...
## 🚨 MANDATORY BUG-CHECKING PROCEDURE 🚨
...
1. Read pipelines/xml-to-sql/docs/BUG_TRACKER.md
2. Read pipelines/xml-to-sql/docs/SOLVED_BUGS.md
...
**Example Lifecycle**:
BUG-029 in tracker → BUG-029 in code → BUG-029 in commits → BUG-029 in solved doc
...
**Historical Note**:
Bugs 001-028 used a dual numbering system (BUG-XXX → SOLVED-XXX) which is now
DEPRECATED. Starting with BUG-029, all bugs keep their original ID permanently.
```

## ⚠️ CONSEQUENCES OF NOT FOLLOWING THIS PROCEDURE
...
**Bug Tracker Location:** `pipelines/xml-to-sql/docs/BUG_TRACKER.md`
**Solved Bugs Location:** `pipelines/xml-to-sql/docs/SOLVED_BUGS.md`

**Process Flow:**
```
HANA Error → Check BUG_TRACKER.md → Check SOLVED_BUGS.md →
Create ticket (if new) → Implement fix → Test → Move to SOLVED_BUGS.md
```
...
## 🚨 MANDATORY SQL ANALYSIS PROCEDURE 🚨
...
File location: LATEST_SQL_FROM_DB.txt (repository ROOT)

BEFORE asking user for SQL:
1. Read LATEST_SQL_FROM_DB.txt from repository root
...
**Latest SQL Location:** `LATEST_SQL_FROM_DB.txt` (repository ROOT)
**Action:** READ IT YOURSELF, NEVER ASK USER TO PASTE

**Process Flow:**
```
Need SQL → Read LATEST_SQL_FROM_DB.txt → Analyze → Reference bug docs →
...
## 🚨 MANDATORY SQL VALIDATION PROCEDURE 🚨
...
## 🚨 MANDATORY POST-GENERATION VALIDATION CHECKS 🚨
...
3. Verify pattern:
   SELECT
      table_alias."COLUMN" AS CC_COLUMN  ← CORRECT (qualified)
   NOT:
      "COLUMN" AS CC_COLUMN              ← WRONG (unqualified)
...
- Makes extensive undocumented changes → breaks working code
- Doesn't test against validated XMLs → ships broken fixes
- Doesn't document changes → impossible to debug
- Touches code that doesn't need touching → introduces new bugs
- BREAKS ALL WORKING FUNCTIONALITY

## 📋 REGRESSION TESTING QUICK REFERENCE

**Golden Commit File:** `pipelines/xml-to-sql/GOLDEN_COMMIT.yaml`
...
**Validation Script:**
```bash
python utilities/validation_script.py --verbose
```

## 🔒 CORE SQL GENERATION FILES - EXTREME CAUTION
...
- `pipelines/xml-to-sql/src/xml_to_sql/sql/function_translator.py` - Function translation
- `pipelines/xml-to-sql/src/xml_to_sql/web/services/converter.py` - View name generation
...
- ✅ Bug-specific changes only (no extras)
- ✅ All changes documented with line numbers
- ✅ All validated XMLs still work (regression test passed)
- ✅ Can enumerate every single change made
- ✅ User can review exactly what changed and why
